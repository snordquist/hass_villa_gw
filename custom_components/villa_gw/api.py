...
Two surfaces:
- HTTP REST via the web admin (`http://<gw>/api/*`) for authenticated queries
- TCP `uart2d:10087` for fire-and-forget bus commands (wake, unlock, …)
- TCP `avlink:10086` for read-only AT+B queries (APPLICATION, SYSTEM, …)
  used for the polling event detector

Optional:
- Telnet `:23` log tail — fast-path for sub-100 ms event delivery, only used
  if the user opts in (and accepts that telnet is open).
"""
